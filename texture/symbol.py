...
def descriptor_symbol(style_layers=['relu1_1', 'relu2_1', 'relu3_1', 'relu4_1', 'relu5_1'], content_layers=['relu4_2'], task='style'):
...
#     style_out = mx.sym.Group([mx.symbol.Dropout(data=x, p=dp) for x in map(eval, style_layers)])
    ...
    if task != 'style':
        return style_out
    content_out = mx.sym.Group(map(eval, content_layers))
    return mx.sym.Group([style_out, content_out])

...
#         pretrained['arg:'+name].copyto(arg_dict[name])
    if name != 'data' and name != 'linear_regression_label':
...
for batch in range(1, 2500000):
    if batch % 5000 == 0:
        ...
        mx.nd.save('args_only_reg37.nd', args_to_save) # avoid device ordinal problem
        mx.nd.save('auxs_only_reg37.nd', auxs_to_save)
    if batch % 50000 == 0:
        optimizer.lr /= 2
...
#     get_data(batch_size, imgout, anno, reg_anno)
    ...
    reg_grad_np = (net.outputs[0] - reg_anno).asnumpy()
...
#        if var.startswith('score') or (var not in pretrained_args and var != 'data'):
        if var != 'data' and var != 'linear_regression_label':
            optimizer.update(i, net.arg_dict[var], net.grad_dict[var], optim_states[i])
    reg_loss += alpha * np.sum(np.square(reg_grad_np)) / np.sum(anno_np[:,0,:,:]==0)
    if batch % 10 == 0:
        print batch, reg_loss/10
        nonzeros, reg_loss = 0, 0
